...
add_features AS (
    SELECT *
        ...
        , TO_CHAR(timestamp,'HH24:MI')::TIME as hour  -- time (hours:minutes) as TEXT data type
        ...
        , DATE_PART('year', timestamp) AS date_yearch
...
add_more_features AS (
    SELECT *
        ,(CASE 
            WHEN hour BETWEEN '00:00' AND '05:00' THEN 'night'
            WHEN hour BETWEEN '06:00' AND '17:00' THEN 'day'
            WHEN hour BETWEEN '18:00' AND '23:00' THEN 'evening'
        END) AS day_part
    FROM add_features
)
SELECT *
